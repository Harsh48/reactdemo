...
axios.defaults.baseURL = 'https://8wjmoib6bi.execute-api.ap-south-1.amazonaws.com/dev/';
...
class TodoStore {
  ...
  @action retrieveTodos = () => {
    axios.get('/todoitems')
      .then(response => {
        ...
        let tempTodos = response.data.body.Items;
  ...
  @action addTodo = event => {
    if (event.key === 'Enter') {
      ...
      const todo_id=Math.floor(1000 + Math.random() * 9000)
      if (todoInput.trim().length === 0) {
        return;
      }
     
      axios.post('/todoitems', {
        message: todoInput,
  ...
  @action deleteTodo = id => {
    axios.delete('/todoitems/' + id)
      .then(response => {
        runInAction(() => {
          const index = this.todos.findIndex(item => item.todo_id.N === id);
